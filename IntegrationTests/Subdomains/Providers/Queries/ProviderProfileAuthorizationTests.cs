...
/// and route authorization policies for the Provider aggregate query endpoints.
/// </summary>
[Collection("SharedDatabase")]
public class ProviderProfileAuthorizationTests : BaseIntegrationTest
{
    public ProviderProfileAuthorizationTests(CustomWebApplicationFactory factory) : base(factory)
    {
    }

    /// <summary>
    /// Ensures that an unauthenticated anonymous user attempting to fetch provider profile details 
    ...
    public async Task GetProviderProfile_ShouldReturnUnauthorized_WhenUserIsAnonymous()
    {
        ...
        var response = await anonymousClient.GetAsync("api/providers/profile", TestContext.Current.CancellationToken);
