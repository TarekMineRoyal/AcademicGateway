...
public class TechSupportAccount : BaseEntity
{
    ...
    /// <exception cref="InvalidTechSupportDetailsException">Thrown if input validation checks fail.</exception>
    public void UpdateStaffDetails(string staffNumber, string supportTier)
    {
        if (string.IsNullOrWhiteSpace(staffNumber))
        {
            throw new InvalidTechSupportDetailsException("Staff number cannot be empty or whitespace.");
        }

        if (string.IsNullOrWhiteSpace(supportTier))
        {
            throw new InvalidTechSupportDetailsException("Support tier assignment level cannot be empty or whitespace.");
        }
