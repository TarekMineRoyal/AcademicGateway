...
/// and exception resilience of the <see cref="ProviderApplicationCreatedEventHandler"/> class.
/// </summary>
public class ProviderApplicationCreatedEventHandlerTests
{
    private readonly Mock<ILogger<ProviderApplicationCreatedEventHandler>> _loggerMock;
    private readonly ProviderApplicationCreatedEventHandler _handler;

    /// <summary>
    /// Initializes a new instance of the <see cref="ProviderApplicationCreatedEventHandlerTests"/> class,
    /// establishing isolated mocking boundaries for infrastructure-level diagnostic dependencies.
    /// </summary>
    public ProviderApplicationCreatedEventHandlerTests()
    {
        _loggerMock = new Mock<ILogger<ProviderApplicationCreatedEventHandler>>();
        _handler = new ProviderApplicationCreatedEventHandler(_loggerMock.Object);
    }
    ...
    [Theory]
    [InlineData("dad957e0-855b-4b12-4ad4-288b47781042", "e1bf627f-1111-4b12-8ad4-288b47781043", "Standard corporate provider registration onboarding draft initialization scenario.")]
        ...
        string? scenario)
    {
        ...
        ProviderApplicationCreatedEvent domainEvent = new(applicationId, providerId);
        ...
        if (exception is null)
        {
            _loggerMock.Verify(
                logger => logger.Log(
                    LogLevel.Information,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((state, type) => state.ToString()!.Contains(applicationId.ToString())),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, exceptionType) => true)),
                Times.Once);
        }
