...
/// Fetches the target project aggregate, executes structural graph sequence checks, and persists shifts safely.
/// </summary>
public class UpdateMilestoneTimelineCommandHandler : IRequestHandler<UpdateMilestoneTimelineCommand, Unit>
{
    ...
    public UpdateMilestoneTimelineCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    /// <summary>
    /// Processes the timeline scheduling update request, ensuring the graph remains compliant with constraint rules.
    /// </summary>
    ...
    /// <exception cref="KeyNotFoundException">Thrown if the project instance root cannot be resolved from database index sweeps.</exception>
    /// <exception cref="InvalidProjectInstanceTransitionException">Thrown if the project state breaks operational updating boundaries.</exception>
    /// <exception cref="InvalidOperationException">Thrown if the proposed timeline shifts violate Directed Acyclic Graph (DAG) constraints.</exception>
